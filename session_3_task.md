...
```python
rows = int(input('Enter number of rowss'))
...
for i in range(n):
    spaces = "  " * (n - 1 - i)  # 2 spaces per row level
    stars = "* " * (2 * i + 1)    # stars with space between
    print(spaces + stars.strip()) # strip() removes trailing space
```
...
# Initialize sum with first term (1)
total = 1

# Calculate remaining terms: x^2/2 + x^3/3 + ... + x^n/n
...
if x <= 0:
    ...
    u = (x - 1) / x  # Common ratio (x-1)/x
    total = 0
    
    # Sum first 7 terms (n = 2 to 7)
    for n in range(2, 8):
        term = (u ** n) / 2
        total += term
    
    print(f"\nSum of first 7 terms: {total}")
    
    # Comparison with actual ln(x)
    import math
    print(f"Actual ln({x}) = {math.log(x)}")
    print(f"Difference: {abs(math.log(x) - total)}")
```
...
# Split into words and take first character of each, converted to uppercase
short_form = ""
...
# Insert second string in middle of first
result = first[:mid] + second + first[mid:]
...
result = lower_chars + upper_chars
print(result)
```
...
# Check if length is even and both halves are equal
if length % 2 == 0 and s[:mid] == s[mid:]:
...
# Uncommon words appear exactly once total (meaning in one sentence only)
...
for i, w in enumerate(words, 1):  # start counting from 1
    if w == word:
        position = i
        found = True
        break
